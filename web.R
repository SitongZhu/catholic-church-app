...
ui <- fluidPage(
  ...
  <p>On this page, users can access detailed Catholic statistics at the country level. 
  These statistics can be filtered by selecting a country of interest. 
  Flexible filters allow users to:</p>

  <ul style='margin-left: 25px;'>
    <li>Choose key variables such as <em>Catholic population</em>, <em>number of priests</em>, 
    or <em>number of parishes</em></li>
    <li>Specify a time period (<em>pre-2010 aggregated data</em> or <em>post-2010 annual data</em>)</li>
    <li>Select a data mode:</li>
      <ul style='margin-left: 25px;'>
        <li><strong>Raw Data</strong> – original published figures</li>
        <li><strong>Statistically Processed Data</strong> – interpolated values for smoother trend analysis</li>
      </ul>
  </ul>
  ...
  <ul style='margin-left: 25px;'>
    <li>The <strong>Bar Chart</strong> compares selected dioceses within the country for the chosen year</li>
    <li>The <strong>Dioceses-Level Trend Analysis</strong> uses line charts to show changes over time</li>
    <li>A complete diocesan data table is available within the Trend Analysis section for detailed reference</li>
  </ul>
  ...
  ")
        ...
        On this page, users can explore <strong>comprehensive Catholic statistics at the diocese level</strong> . 
        These statistics can be filtered by selecting a specific <strong>country</strong> and <strong>diocese of interest</strong>. 
        The available filters allow users to:
...
<ul style='font-size: 18px; line-height: 1.6; color: #2c3e50; font-family: Arial, sans-serif;'>
  ...
  <li>Specify a time period (<em>pre-2010 aggregated data</em> or <em>post-2010 annual data</em>)</li>
  <li>Select a data mode:
    <ul>
      <li>Raw Data – original figures as published</li>
      <li>Statistically Processed Data – interpolated values for smoother trend analysis</li>
    </ul>
  </li>
</ul>
<p style='font-size: 18px; line-height: 1.6; color: #2c3e50; font-family: Arial, sans-serif;'>
Interactive charts and tables enable users to visualize diocesan development trends and 
explore data patterns intuitively, providing both high-level insights and detailed statistical context.
      </p>")
                    )
                  )
                ),
...
server <- function(input, output, session) {
  ...
  output$trendTableDescription <- renderUI({
    ...
    country_text <- if (is.null(input$countries) || length(input$countries) == 0) {
      ...
      paste("the selected countries (<strong>", paste(input$countries, collapse = ", "), "</strong>)", sep = "")
    }
  ...
  output$dl_barChartDescription <- renderUI({
    ...
    HTML(paste0(
      "<p style='font-size: 18px; line-height: 1.6; color: #2c3e50; font-family: Arial, sans-serif;'>",
      data_description, " ", var_text," across the years for the <strong>Diocese of ", input$dl_diocese, "</strong>, located in <strong>", diocese_country, "</strong>. ",
  ...
  output$download_cl_trend_png <- downloadHandler(
    filename = function() {
      ...
      paste("cl_trend-", input$cl_var, "-", input$cl_country, "-", tp, "-", Sys.Date(), ".png", sep = "")
    },
    content = function(file) {
  ...
  output$download_cl_trend_pdf <- downloadHandler(
    filename = function() {
      ...
      paste("cl_trend-", input$cl_var, "-", input$cl_country, "-", tp, "-", Sys.Date(), ".pdf", sep = "")
    },
    content = function(file) {
      tr <- cl_trend_reactive()
      if (is.null(tr) || nrow(tr) == 0) {
  ...
  output$download_cl_trend <- downloadHandler(
    filename = function() {
      ...
      paste("cl_trend-", input$cl_var, "-", input$cl_country, "-", tp, "-", Sys.Date(), ".jpg", sep = "")
    },
    content = function(file) {
      tr <- cl_trend_reactive()
      if (is.null(tr) || nrow(tr) == 0) {
  ...
  output$cl_barChartDescription <- renderUI({
    ...
    HTML(paste0(
      "<p style='font-size: 18px; line-height: 1.6; color: #2c3e50; font-family: Arial, sans-serif;'>",
      data_description, " for <strong>", diocese_text, "</strong> in <strong>", input$cl_country, "</strong>,
  ...
  output$cl_trendChartDescription <- renderUI({
    ...
    HTML(paste0(
      ...
      "The chart enables users to observe growth, decline, or stability of the selected statistical metric. ",
      "The data presented here covers the time period ", time_period_text, " and is based on <strong>", mode_text, "</strong>.",
      "</p>"
    ))
  })
